...
client.on("message", async (message) => {
  ...
  // /* Insultons Tybo ! */
  // else if (message.content.startsWith(`${prefix}chauve`)) {
  //   if(message.author.id === '221995632079011841') {
  //     message.channel.send("T'es sûr t'es hpi espèce de débile ???");
  //     client.users.fetch('221995632079011841', false).then((user) => {
  //       user.send("Nan vraiment, t'es stupide hein");
  //      });
  //   } else {
  //     message.channel.send("On vient de me convoquer pour te dire de bien aller te faire enculer <@221995632079011841>, sale chauve de merde.");
  //     client.users.fetch('221995632079011841', false).then((user) => {
  //       user.send(`Psssst, quelqu'un vient de me demander de t'insulter, il s'agit de ${message.author.username}${message.author.discriminator}`);
  //      });
  //   }
  //   return;
  // }
  ...
  // /* Insultons Val ! */
  // else if (message.content.startsWith(`${prefix}val`)) {
  //   if(message.author.id === '291605174517235712') {
  //     message.channel.send("Le mec confirme la règle disant que les policiers sont cons");
  //     client.users.fetch('291605174517235712', false).then((user) => {
  //       user.send("Nan vraiment, t'es stupide hein");
  //      });
  //   } else {
  //     message.channel.send("Je suis témoin : <@291605174517235712> suce des maxi-beuteus !");
  //     client.users.fetch('291605174517235712', false).then((user) => {
  //       user.send(`Ramène ton flingue on a besoin de toi pour une intervention chez ${message.author.username}${message.author.discriminator}, il a demandé à t'insulter !`);
  //      });
  //   }
  //   return;
  // }
  ...
  // /* Insultons Platy ! */
  // else if (message.content.startsWith(`${prefix}platy`)) {
  //   if(message.author.id === '186150008809193472') {
  //     message.channel.send("Pas malin ça bebou...");
  //     client.users.fetch('186150008809193472', false).then((user) => {
  //       user.send("On fait tous des erreurs bebou, évite de recommencer d'accord...");
  //      });
  //   } else {
  //     message.channel.send("Ferme ta gueule <@186150008809193472>, t'es vrm trop cringe !");
  //     client.users.fetch('186150008809193472', false).then((user) => {
  //       user.send(`Bebou j'suis désolée... ${message.author.username}${message.author.discriminator} m'a forcé à le faire, j'le pensais pas... :(`);
  //      });
  //   }
  //   return;
  // }
  else if (message.content.startsWith(`${prefix}sex`)) {
    client.users.fetch(message.author.id, false).then((user) => {
      user.send(
        `Envie de sexe ? Tiens ça va te calmer : ||https://pornypon.com/||`
      );
    });
    message.channel.send(ahegao);
    return;
  } else if (message.content.startsWith(`${prefix}sus`)) {
    message.channel.send(
      `⠀⠀⠀⠀⠀       ⢰⡿⠋⠁⠀⠀⠈⠉⠙⠻⣷⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
        ⠀⠀⠀⠀⢀⣿⠇⠀⢀⣴⣶⡾⠿⠿⠿⢿⣿⣦⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
        ⠀⠀⣀⣀⣸⡿⠀⠀⢸⣿⣇⠀⠀⠀⠀⠀⠀⠙⣷⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    message.channel.send("susmabit oue");
    return;
  } else if (message.content.startsWith(`${prefix}SUS`)) {
    message.channel.send(
      `⠀⠀⠀⠀⠀       ⢀⣴⡾⠿⠿⠿⠿⢶⣦⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
